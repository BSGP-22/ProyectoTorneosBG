...
-- Creacion
select * from ARBITRO
create proc insertar_arbitros @DPI int, @nombre varchar(200),@apellido varchar(200),@direccion varchar(200),@telefono varchar(200),@nacionalidad varchar(200),@fechaNac date,@correo varchar(200),@rol varchar(200),@arbitraje decimal (8, 2)
as
	insert into ARBITRO
    (
...
create proc actualizar_arbitros @DPI int, @nombre varchar(200),
    ...
    @fechaNac date,@correo varchar(200),@rol varchar(200),@arbitraje decimal (8, 2) 
as
    update ARBITRO set 
        Nombre= @nombre, Apellidos= @apellido, 
        Direccion= @direccion, Telefono= @telefono, 
        Nacionalidad=@nacionalidad, FechaNac=@fechaNac,
        Correo= @correo ,Rol= @rol, Arbitraje=@arbitraje where DPI= @DPI
go
-- Eliminacion
...
Usuario Varchar (200) default 'Vacio',
accion Varchar (200) default 'Vacio',
fecha datetime default getdate()
);

--insertando Bitacora procedimiento almacenado
create proc BG_Insetar_Datos_Bitacora @Usuario varchar(200),@accion varchar(200)
as
insert into ControlBitacora(Usuario,accion) values(@Usuario,@accion)

exec BG_Insetar_Datos_Bitacora

select  * from ControlBitacora 

select * from cancha
select * from ARBITRO
select * from ALQUILER
insert Into ALQUILER values ('Bryan','Perez',3001,'4545','correo','2021/05/19',1,'9:00','10:00',3011,155,155)
insert Into ALQUILER values ('Steve','Perez',555,'123','prueba','2021/05/19',1,'11:00','12:00',3011,155,155)
--Reporte de bitacoda 

create proc BG_Reporte_Bitacora @fechaInicio datetime, @fechaFin datetime, @Usuario varchar(200)
as
Select * from ControlBitacora where  (CONVERT(date,fecha)>=@fechaInicio and CONVERT(date,fecha)<=@fechaFin)  and Usuario=@Usuario

select * from alquiler
--exec BG_Reporte_Bitacora '2021/05/19','2021/05/19','smendez'
...
--creando reporte de cancha
	...
	group by alquiler.No_Cancha,CANCHA.Nombre,year(fecha),MONTH(fecha),DAY(fecha)
	order by MONTH(fecha)asc
--EXEC BG_reporte_Ingresos_canchas @fecha_inico='2021/05/19',@fecha_final='2021/07/30'
go
...
create proc  BG_reporte_Ingreso_arbitro @fecha_inico date,@fecha_final date
	...
	where fecha between @fecha_inico  and @fecha_final and arbitro.DPI<>0
	group by alquiler.DPI_Arbitro,arbitro.Nombre,arbitro.Apellidos,year(fecha),MONTH(fecha),DAY(fecha)
	order by MONTH(fecha)asc
--EXEC BG_reporte_Ingreso_arbitro @fecha_inico='2021/01/01',@fecha_final='2021/07/30'

-- Procedimiento para Mostrar Horarios 
create proc BG_Mostrar_Horarios 
as 
Select * from Horarios

--Exec BG_Mostrar_Horarios
insert horarios(Horario_lunes_inicio) values('00:00')
--Procedimiento para actualizar Horarios 

create proc BG_Actualizar_horarios @No_Horario int ,@Horario_lunes_inicio Time,@Horario_lunes_fin Time, @Horario_Martes_inicio Time, @Horario_Martes_fin Time, @Horario_Miercoles_inicio Time, @Horario_Miercoles_fin Time, @Horario_Jueves_inicio Time, @Horario_Jueves_fin Time, @Horario_Viernes_inicio Time, @Horario_Viernes_fin Time, @Horario_Sabado_inicio Time, @Horario_Sabado_fin Time, @Horario_Domingo_inicio Time, @Horario_Domingo_Fin time
as
update Horarios set Horario_lunes_inicio=@Horario_lunes_inicio, Horario_lunes_fin=@Horario_lunes_fin ,Horario_Martes_inicio=@Horario_Martes_inicio,Horario_Martes_fin=@Horario_Martes_fin,Horario_Miercoles_inicio=@Horario_Miercoles_inicio, Horario_Miercoles_fin=@Horario_Miercoles_fin,Horario_Jueves_inicio=@Horario_Jueves_inicio,Horario_Jueves_fin=@Horario_Jueves_fin, Horario_Viernes_inicio=@Horario_Viernes_inicio,Horario_Viernes_fin=@Horario_Viernes_fin,Horario_Sabado_inicio=@Horario_Sabado_inicio,Horario_Sabado_fin=@Horario_Sabado_fin,Horario_Domingo_inicio=@Horario_Domingo_inicio,Horario_Domingo_Fin=@Horario_Domingo_Fin   where No_Horario=@No_Horario

--Exec BG_Actualizar_horarios

--Mostrar Todos los Usuarios 
Create Proc BG_Mostrar_Usuarios
as
select * from Usuarios
...
(
NOMBRE,APELLIDO,DPI,DIRECCION,PUESTO,TELEFONO,CORREO,USUARIO,CLAVE,ACTIVO,Rol
)
Values 
(
@NOMBRE,@APELLIDO,@DPI,@DIRECCION,@PUESTO,@TELEFONO,@CORREO,@USUARIO,@CLAVE,@ACTIVO,@Rol
...
--exec BG_Insertar_Usuarios

--Procedimiento para actualizar Usuarios 
--drop proc BG_Actualizar_Usuarios
Create Proc BG_Actualizar_Usuarios @Nombre varchar(200),@Apellido varchar(200), @DPI bigint, @Direccion varchar(200), @Puesto varchar(200), @Telefono varchar(200), @Correo varchar(200),@Usuario varchar(200),@Clave varchar(200), @Activo varchar(1),@Rol varchar(200)
as
Update Usuarios set 
Nombre= @Nombre, APELLIDO= @apellido, DIRECCION=@Direccion, PUESTO=@Puesto, TELEFONO=@Telefono, CORREO=@Correo, USUARIO=@Usuario, CLAVE=@Clave, ACTIVO=@Activo, Rol=@Rol where DPI=@DPI

--Exec BG_Actualizar_Usuarios


--Procedimiento Para Reporte de Disponibilidad de canchas 


Create proc BG_Reporte_Disponibilidad @FechaInicio date, @FechaFin date,@NombreCancha Varchar(200)
as

select c.NoCancha,c.Nombre,a.fecha,a.HORAINICIO,a.HORAFINAL from CANCHA c
	left join alquiler a
	on c.NoCancha=a.No_Cancha
	group by c.NoCancha,c.Nombre, a.FECHA,a.HORAINICIO,a.HORAFINAL
	having  a.fecha between @FechaInicio and @FechaFin and c.Nombre=@NombreCancha
	

--Exec BG_Reporte_Disponibilidad
create proc BG_canchas_disponibles @fecha date, @Horainicio time, @Horafinal time
as
select*from  CANCHA
...
where CONVERT(date,p.Fecha)=@fecha and (CONVERT(time,HoraInicio)>=@Horainicio and   CONVERT(time,HoraFinal)<=@Horafinal)
)
and NoCancha not in
(select No_Cancha from alquiler
where fecha=@fecha and HORAINICIO>=@Horainicio and HORAFINAL<=@Horafinal
)
and estatus='Habilitada'
--Exec BG_canchas_disponibles

--Mostrar arbitros disponibles
create proc vista_arbitros_disponibles @fecha date,@HoraInicio TIME,@HoraFinal TIME
	as 
select*from  ARBITRO
where DPI  not in(
...
and DPI  not in
(select DPI_Arbitro from alquiler
where fecha=@fecha and HORAINICIO>=@HoraInicio and HORAFINAL<=@HoraFinal
)
and DPI<>0
...
create proc BG_Mostrar_Alquiler
as
    select * from alquiler
go 
-- Exec BG_Mostrar_Alquiler

-- Insertar datos a nuestra tabla Alquiler 

create proc BG_Insert_Alquiler  @nombre varchar(200),@apellido varchar(200), @DPI bigint, @telefono varchar(200),@Correo varchar(200),@fecha date,@No_Cancha int,@HORAINICIO time,@HORAFINAL time, @DPI_Arbitro int, @PAGO Decimal (8,2), @PAGOARBITRO Decimal (8,2)
as
	insert into alquiler
    (
        nombre,apellido,DPI,telefono,
        Correo,fecha,No_Cancha,HORAINICIO,HORAFINAL,DPI_Arbitro,PAGO,PAGOARBITRO
    )
    values
    (
        @nombre,@apellido,@DPI,@telefono,
